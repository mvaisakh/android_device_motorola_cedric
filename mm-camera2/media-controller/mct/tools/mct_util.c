  ...
  struct timespec *ts)
{
  ...
  signed long long wait_time =
    ((signed long long)(timeout_interval) * SEC_TO_NS_FACTOR);
  if (!ts) {
  ...
  end_time = (((((signed long long)now.tv_sec) * 1000000) + now.tv_usec) +
    (wait_time / 1000));
 ...
 *    create the timer
 **/
int mct_util_create_timer(timer_t *mct_timerid)
{
 ...
 *    set the timer
 **/
void mct_util_set_timer(struct timespec *timeToWait,timer_t *mct_timerid)
{
    struct itimerspec mct_in, mct_out;
 ...
 *    delete the timer
 **/
int mct_util_delete_timer(timer_t *mct_timerid)
{
    int mct_t_ret = 0;
    mct_t_ret = timer_delete(mct_timerid) ;
    return mct_t_ret;
}
