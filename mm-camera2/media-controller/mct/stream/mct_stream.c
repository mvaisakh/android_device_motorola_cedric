...
/*  ZSL Snapshot case, where PPROC is part of Offline stream*/
  ...
  case CAM_STREAM_TYPE_RAW: {
    CLOGI(CAM_MCT_MODULE, "Start linking raw stream 0x%x",
      stream->streaminfo.identity);
...
static uint32_t
mct_stream_util_check_streamid_exists(cam_stream_ID_t streamids,
                                      unsigned int streamid)
{
  uint32_t i;
  for (i = 0; i < streamids.num_streams; i++) {
    if (streamids.streamID[i] == streamid) {
      return 1;
    }
  }
  return 0;
 ...
 *    @stream_id: Optional check for matching stream present
 *    @modify: If set to TRUE, contents of matched entry are removed
 *    @stream_check: If set to true, check for matching stream_id
 *  Return: If valid frame_number found return 1 else return 0
 *
 *  Description:
 *  This function checks for matching frame_id in a list storing
 *  corresponding frame_nums issued by HAL. If found, frame_number
 *  is returned to calling function.
 **/

unsigned int mct_stream_get_frame_number(mct_pipeline_t*pipeline,
  unsigned int frame_id, unsigned int *frame_number, unsigned int stream_id,
  boolean modify, boolean stream_check)
  {
  ...
  if (holder && holder->data) {
    ...
    if ((stream_check) &&
        (mct_stream_util_check_streamid_exists(data->stream_ids, stream_id))) {
      *frame_number = data->frame_number;
      frame_number_found = 1;
    } else if (!stream_check) {
      *frame_number = data->frame_number;
      frame_number_found = 1;
    ...
    case MCT_BUS_MSG_ISP_SOF:
    {
      ...
      if (stream->metadata_stream.get_buf_err == FALSE) {
        ...
        urgent_frame_num_valid = mct_stream_get_frame_number (
          pipeline, urgent_frame_id, &meta_urgent_frame_number,
          stream->streamid, FALSE, FALSE);
        frame_num_valid = mct_stream_get_frame_number (
          pipeline, isp_sof_bus_msg->frame_id, &meta_frame_number,
...
static boolean mct_stream_destroy_buffers(void *data, void *user_data)
{
  ...
  for (i = 0; i < mbuf->num_planes; i++) {
    ...
    if (stream && stream->streaminfo.is_secure == SECURE) {
      close(mbuf->buf_planes[i].fd);
    }
#endif
  }
...
void *get_pointer_of(cam_intf_parm_type_t meta_id, metadata_buffer_t* metadata)
{
  switch(meta_id) {
    ...
    case CAM_INTF_PARM_LED_CALIBRATION:
      return POINTER_OF_META(CAM_INTF_PARM_LED_CALIBRATION, metadata);
    ...
    case CAM_INTF_PARM_TEST_DUALLED_VALUE:
      return POINTER_OF_META(CAM_INTF_PARM_TEST_DUALLED_VALUE, metadata);
...
uint32_t get_size_of(cam_intf_parm_type_t param_id)
{
  ...
  switch(param_id) {
    ...
    case CAM_INTF_PARM_LED_CALIBRATION:
      return SIZE_OF_PARAM(CAM_INTF_PARM_LED_CALIBRATION, metadata);
    case CAM_INTF_PARM_INITIAL_EXPOSURE_INDEX:
	...
	/* MODIFIED-BEGIN by yongzhi.li, 2016-09-21,BUG-2579753*/
	case CAM_INTF_PARM_TEST_DUALLED_VALUE:
      return SIZE_OF_PARAM(CAM_INTF_PARM_TEST_DUALLED_VALUE, metadata);
      /* MODIFIED-END by yongzhi.li,BUG-2579753*/
    case CAM_INTF_META_FOCUS_DEPTH_INFO:
