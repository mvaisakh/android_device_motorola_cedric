  ...
  cam_stream_type_t stream_type)
{
  ...
  if (stream_type == CAM_STREAM_TYPE_OFFLINE_PROC) {
    ...
    for (i = 0; i < session_params->valid_stream_ids[q_idx].num_streams; i++) {
      if (session_params->valid_stream_ids[q_idx].streamID[i] == stream_id) {
        session_params->valid_stream_ids[q_idx].streamID[i] = 0;
        rc = 1;
        break;
      }
    }
  ...
  cpp_module_process_list_t *hw_process_list)
{
 ...
 /* linked stream case */
  ...
  cpp_module_decide_hw_proc_info(module, &stream_parm_list[0],
    stream_list_size, hw_process_list);
  ...
  cpp_module_process_list_t hw_process_list)
{
  ...
  for (i = 0; i < hw_process_list.num_hw_process; i++) {
    ...
    clk_rate_obj->duplication_flag = FALSE;
    ...
    if (hw_proc_info->dup_identity != 0) {
      ...
      if (linked_clk_rate_obj != NULL) {
        linked_clk_rate_obj->duplication_flag = TRUE;
        clk_rate_obj->duplication_flag = TRUE;
      }
    }
  }
  ...
  uint32_t identity, isp_buf_divert_t *isp_buf)
{
  ...
  if (ret < 0) {
    CPP_BUF_LOW("Send inplace ACK for frame %d %x", frame_id, identity);
  ...
  for (j = 0; j < hw_process_list.num_hw_process; j++) {
    ...
    if (div_identity == 0 && hw_proc_info->dup_identity != 0) {
  ...
  if (ctrl->cpp_thread_started) {
    if (num_events != 0) {
      ...
      isp_buf->ack_flag = TRUE;
      isp_buf->is_buf_dirty = 1;
      CPP_BUF_LOW("buffer event received with no divert config\n");
      PTHREAD_MUTEX_UNLOCK(&(ctrl->cpp_mutex));
      MCT_OBJECT_UNLOCK(stream_port);
      return 0;
    }
  ...
  mct_event_t* event)
{
  ...
  if ((!hw_params->input_info.width) ||
    ...
    CPP_CROP_HIGH("Crop event recieved before output dimension");
    return 0;
  }
  ...
  mct_event_t* event)
{
    ...
    if(stream_params->stream_type != CAM_STREAM_TYPE_OFFLINE_PROC) {
      ...
      for (i = 0; i < CPP_MODULE_MAX_STREAMS; i++) {
        if (session_params->stream_params[i]) {
          session_params->stream_params[i]->module_chromatix =
            *chromatix_param;
        }
      }
    } else {
      ...
      if (parm_data) {
        ...
        *(cam_stream_ID_t *)parm_data =
        ...
        for (j = 0; j <
          ...
          CPP_PER_FRAME_DBG("META_STREAM_ID q_idx %d frame id %d streamID:%d",
            q_idx, future_frame_id,
            ((cam_stream_ID_t *)control_param->parm_data)->streamID[j]);
        }
        cpp_module_util_check_per_frame_limits(ctrl, identity,
          future_frame_id, parm_data);
      }
      break;
        ...
        if (MCT_EVENT_MODULE_EVENT == frame_ctrl_data->mct_type) {
          ...
          if ((rc == 0) && (param_processed == TRUE) &&
            ...
            /* The parameter that got just applied needs to be reported depending
               ...
               event for reporting */
            report_q_idx = cur_frame_id + per_frame_params->max_report_delay;
            if (report_q_idx > per_frame_params->cur_frame_id){
              cpp_module_add_report_entry(per_frame_params,
                frame_ctrl_data,
                cur_frame_id + per_frame_params->max_report_delay, identity);
            } else if (report_q_idx == per_frame_params->cur_frame_id) {
  ...
  boolean *param_processed, uint32_t current_frame_id)
{
  ...
  case CAM_INTF_PARM_HAL_VERSION: {
    ...
    cpp_module_session_params_t* session_params = NULL;
    cpp_module_stream_params_t* stream_params = NULL;
    CPP_HIGH("CAM_INTF_PARM_HAL_VERSION, value=%d, identity=0x%x",
      value, identity);
    /* get parameters based on the event identity */
  ...
  mct_event_t* event)
{
  ...
  cpp_module_set_clock_freq(ctrl, module, stream_params, 1,session_params);
  ...
  defined (CHROMATIX_310) || defined (CHROMATIX_310E)
    if (!stream_params->module_chromatix.chromatixCppPtr) {
      CPP_ERR("failed chromatix_param is NULL\n");
      goto CPP_MODULE_STREAMON_ERROR2;
    }
#else
    if (!stream_params->module_chromatix.chromatixPtr) {
      CPP_ERR("failed chromatix_param is NULL\n");
      goto CPP_MODULE_STREAMON_ERROR2;
    }
#endif
  ...
  mct_event_t* event)
{
  ...
  cpp_module_set_clock_freq(ctrl, module, stream_params, 0, session_params);
