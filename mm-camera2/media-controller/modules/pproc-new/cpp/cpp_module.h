...
#define CPP_WAIT_TIMEOUT 300000000
...
#define IS_HW_VERSION_SUPPORTS_ROTATION(ctrl) \
  (ctrl->cpphw->hwinfo.version != CPP_HW_VERSION_6_1_0) ? 1 : 0 \
...
typedef struct _cpp_module_stream_clk_rate_t {
  uint32_t identity;
  uint64_t total_load;
  boolean duplication_flag;
  boolean tnr_on_flag;
  boolean dsdn_on_flag;
  float   system_overhead;
  boolean   ubwc_on_flag;
