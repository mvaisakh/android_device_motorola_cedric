    ...
    case MCT_EVENT_CONTROL_STREAMOFF: {
      ...
      if(p_client->processing_mask) {
        if (p_mod->modparams.imgbase_client_pre_streamoff) {
          p_mod->modparams.imgbase_client_pre_streamoff(p_client, stream_info->identity);
        }
      } else {
        IDBG_ERROR("Processing mask absent!");
      }
  ...
  case MCT_EVENT_MODULE_EVENT: {
    ...
    case MCT_EVENT_MODULE_BUF_DIVERT: {
      ...
      if (p_client->state == IMGLIB_STATE_ERROR ||
        p_client->state == IMGLIB_STATE_STOP_PENDING) {
        IDBG_ERROR("%s:%d] Invalid state to handle divert state %d pending 5 error 6",
      ...
      IDBG_MED("[%s] [IMG_BUF_DBG_IN_f%d_b%d_i%x] buf_div str_to_proc %x",
        p_mod->name,
