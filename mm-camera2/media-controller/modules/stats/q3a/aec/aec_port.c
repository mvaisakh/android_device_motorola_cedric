  ...
  aec_output_data_t *output)
{
  ...
  private->aec_info.est_snap_luma_index_for_hdr = output->stats_update.aec_update.lux_idx; // MODIFIED by chen.cao, 2016-10-21,BUG-3182413
  ...
  aec_port_peer_aec_update* master_update_info)
{
  ...
  AEC_LOW("Using adjusted multiplier value of %f at %dK temperature for mono interpolation", multiplier, awb_colortemp);
  ...
  stats_update_t*           slave_stats_update)
{
  ...
  AEC_LOW("\nDual interpolation: Master exp time %f gain %f linecount %d exp_index: %d, lux_idx: %f\n"
          "Dual interpolation: Slave  exp time %f gain %f linecount %d exp_index: %d, lux_idx: %f",
    ...
    if (stat_parm->param_type == STATS_SET_Q3A_PARAM) {
      ...
      if (q3a_param->type == Q3A_SET_AEC_PARAM) {
        ...
        case Q3A_ALL_SET_DO_LED_EST_FOR_AF: {
          ...
          if (q3a_param->u.q3a_all_param.u.est_for_af) {
      ...
      if (TRUE == rc) {
        ...
        if (private->stream_type == CAM_STREAM_TYPE_SNAPSHOT) {
          ...
          if (private->manual.is_exp_time_valid &&
            ...
            real_gain = private->manual.gain;
            linecount = (out->real_gain[0] *
                         out->linecount[0])/ real_gain;
            exp_time =  (out->real_gain[0] *
                         out->exp_time) / real_gain;
            sensor_gain = real_gain;
          }
          if (linecount < 1) {
            linecount = 1;
