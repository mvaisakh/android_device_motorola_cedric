...
typedef struct
{
  /* peer ID for the linked camera, or 0 if not linked */
  uint32_t            peer_id;

  /* Indicates whether the OTP data valid */
  boolean             otp_data_valid;

  /* Reference AWB color temperator in Kelvin in which the compensation ratio is tuned */
  uint16_t            tuned_color_temp;

  /* Exposure multiplier, from OTP data, to interpolate bayer AEC parameters for mono*/
  float               exp_multiplier_otp;

  /* Exposure multiplier, from the tuning headers; only used if OTP data is not present */
  float               exp_multiplier_tuning;

  /* Dual camera info to indicate which sensor this is */
  cam_sync_type_t     sensor_type;

  /* Dual camera calibration ratios for different color temperatures */
