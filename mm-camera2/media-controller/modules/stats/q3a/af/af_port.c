  ...
  cam_position_t cam_position, unsigned int *sessionid)
{
  ...
  if (FALSE == private->af_extension_use) {
    ...
    private->af_iface_lib = af_port_ext_load_function(&private->af_object.af_ops,
      q3a_session_data->af_libptr, cam_position, private->use_default_algo);
  }
  ...
  private->syscalib_data.isValid = session_data->is_valid_dualcalib;
  if (private->syscalib_data.isValid) {
    Q3A_MEMCPY(&private->syscalib_data.af_syscalib_data,
      &session_data->dual_data,sizeof(cam_related_system_calibration_data_t));
  }
...
static void af_port_lock_caf(af_port_private_t *af_port, boolean lock)
{
  ...
  q3a_thread_af_msg_t *af_msg =
    (q3a_thread_af_msg_t *)malloc(sizeof(q3a_thread_af_msg_t));
  if (af_msg == NULL) {
...
boolean af_port_forward_focus_info_if_linked(mct_port_t  *port) {
  ...
  boolean rc = TRUE;
  if ((af_port->dual_cam_sensor_info == CAM_TYPE_MAIN) &&
    (af_port->intra_peer_id != 0)) {
    rc = stats_util_post_intramode_event(port,af_port->intra_peer_id,
      MCT_EVENT_MODULE_STATS_PEER_AF_FOCUS_UPDATE,&af_port->af_peer_focus_info);
  }
  return rc;
}
...
static boolean af_port_send_wakeup(mct_port_t *port, boolean enable)
{
  ...
  if (enable == TRUE) {
    AF_LOW("Send AF_WAKEUP sID=%d CAM-TYPE=%d", (af_port->reserved_id >> 16),
      af_port->dual_cam_sensor_info);
    req_frame_msg.request_flag = TRUE;
    req_frame_msg.req_mode = FRM_REQ_MODE_CONTINUOUS;
  } else {
    AF_LOW("Send AF_SLEEP sID=%d CAM-TYPE=%d", (af_port->reserved_id >> 16),
      af_port->dual_cam_sensor_info);
...
boolean af_port_wake_up_if_required(mct_port_t  *port,
  ...
  if((af_port->focusedAtLeastOnce == FALSE) ||
    (af_port->af_master_focus_info.focusStatus != info->focusStatus)) {
    AF_LOW("WakeupRequired Master Prev and Cur Focus Status=[%d %d] "
      "CAM-TYPE=%d FocussedatLeastOnce=%d",
      af_port->af_master_focus_info.focusStatus, info->focusStatus,
      af_port->dual_cam_sensor_info,
...
boolean af_port_handle_peer_focus_info(mct_port_t  *port,
  ...
  af_port->af_master_focus_info = *info;
  ...
  memset(af_msg, 0, sizeof(q3a_thread_af_msg_t));
  af_msg->type = MSG_AF_SET;
  af_msg->u.af_set_parm.type = AF_SET_PARAM_SET_PEER_FOCUS_INFO;
...
/** af_port_set_follow_mode
 * Set AF Algorithm to a follow mode
 *
 * @port: port info
 *
 ...
 * Return: True - Success  False - failure
 **/
boolean af_port_set_follow_mode(mct_port_t  *port,
  boolean followMode) {
  af_port_private_t *af_port = (af_port_private_t *)(port->port_private);
  boolean rc = FALSE;
  ...
  memset(af_msg, 0, sizeof(q3a_thread_af_msg_t));
  af_msg->type = MSG_AF_SET;
  af_msg->u.af_set_parm.type = AF_SET_PARAM_SET_FOLLOW_MODE;
  af_msg->u.af_set_parm.u.af_follow_mode = followMode;
...
boolean af_port_update_af_state(mct_port_t  *port,
  ...
  if (new_state != af_port->af_trans.af_state) {
    ...
    af_port->focus_state_changed = TRUE;
    pthread_mutex_unlock(&(af_port->focus_state_mutex));
    AF_LOW("Af State changed CAM-TYPE=%d new_state=%d, old_state=%d!, cause=%d, af_mode=%d",
      af_port->dual_cam_sensor_info,
...
boolean af_port_process_status_update(
  ...
  if(algo_status_changed) {
    AF_LOW("CAM-TYPE=%d Status Update from algo! %d",
      af_port->dual_cam_sensor_info,
      af_status->status);
  }
  ...
  af_port->af_peer_focus_info = af_out->af_peer_focus_info;
  ...
  af_output_data_t *af_out)
{
  ...
  if((af_out->swaf_config.enable == FALSE) &&
    (af_port->dual_cam_sensor_info == CAM_TYPE_AUX)) {
    AF_LOW("sending AF_SLEEP from %d",af_port->dual_cam_sensor_info);
...
static void af_port_callback(af_output_data_t *af_out, void *p)
{
  ...
  if (af_out->result) {
    AF_LOW("output mask= %x", af_out->type);
  ...
  if (send_update) {
    ...
    AF_LOW("send MCT_EVENT_MODULE_STATS_AF_UPDATE to af port,"
      " port =%p, event =%p", port, &event);
...
/** af_port_set_stereo_calibration_data:
 *    @af_port: private port data
 *
 * Copy the stereo calibration data to algorithm.
 *
 * Return void
 **/
static void af_port_set_stereo_calibration_data(af_port_private_t *af_port,
  af_core_calibration_data *p_syscalib_data)
{
  memcpy(&af_port->syscalib_data.af_syscalib_data, p_syscalib_data,
  ...
  if (af_msg != NULL) {
    memset(af_msg, 0, sizeof(q3a_thread_af_msg_t));
    af_msg->type = MSG_AF_SET;
    af_msg->u.af_set_parm.type = AF_SET_PARAM_SET_STEREO_CAL_DATA;
    af_msg->u.af_set_parm.u.p_af_syscalib_data = &p_syscalib_data->af_syscalib_data;
    AF_LOW("Stereo calibration fl:%f cw:%d ch:%d nw:%d nh:%d CAM-TYPE=%d",
      p_syscalib_data->af_syscalib_data.aux_cam_specific_calibration.normalized_focal_length,
      p_syscalib_data->af_syscalib_data.aux_cam_specific_calibration.calibration_sensor_resolution_width,
      p_syscalib_data->af_syscalib_data.aux_cam_specific_calibration.calibration_sensor_resolution_height,
      p_syscalib_data->af_syscalib_data.aux_cam_specific_calibration.native_sensor_resolution_width,
      p_syscalib_data->af_syscalib_data.aux_cam_specific_calibration.native_sensor_resolution_height,
      af_port->dual_cam_sensor_info);
    q3a_af_thread_en_q_msg(af_port->thread_data, af_msg);
  }
}

/** af_port_get_stereo_calibration_data:
 *    @af_port: private port data
 *
 * Copy the stereo calibration data to peer.
 *
 * Return void
 **/
static void af_port_get_stereo_calibration_data(af_port_private_t *af_port,
  cam_related_system_calibration_data_t *p_syscalib_data)
{
  AF_HIGH("Get stereo calibration data cam:%d!",af_port->dual_cam_sensor_info);

  memcpy(p_syscalib_data, &af_port->syscalib_data,
    sizeof(cam_related_system_calibration_data_t));
}
...
void af_port_set_haf_enable(af_port_private_t *af_port)
{
  ...
  memset(af_msg, 0, sizeof(q3a_thread_af_msg_t));
  af_msg->type = MSG_AF_SET;
  af_msg->u.af_set_parm.type = AF_SET_PARAM_HAF_ENABLE;
  ...
  mct_event_module_t *mod_evt)
{
  ...
  tuning_info = &((chromatix_3a_parms_type *)mod_chrom->chromatix3APtr)->AF_algo_data;

  af_algo_tune_parms_adapter_t *tuning_info_adapter =
    &af_port->tuning_adapter_info;
  memset(tuning_info_adapter, 0, sizeof(*tuning_info_adapter));

  (void)parse_AF_chromatix(tuning_info, tuning_info_adapter);
  ...
  if (af_msg != NULL) {
    ...
    af_msg->u.af_set_parm.u.af_init_param.tuning_info = tuning_info_adapter;

    /* Enable/Disable HAF based on tuning data and internal capabilities */
    af_port->af_haf_tuning = &tuning_info_adapter->af_algo.af_haf;
    ...
    af_port_populate_reserve_tuning_params(af_port,
      &af_msg->u.af_set_parm.u.af_init_param.core_tuning_params, tuning_info_adapter);

    q3a_af_thread_en_q_msg(af_port->thread_data, af_msg);

    /* Also update tuning header for fd_prio AF */
    af_port->fd_prio_data.tuning_info = tuning_info_adapter;
    ...
    af_tuning_lens_sag_comp_adapter_t *tuning_ptr = &(tuning_info_adapter->af_algo.lens_sag_comp);
...
static void af_port_handle_current_pos_update_evt(af_port_private_t *af_port, int current_position)
{
  ...
  if (af_msg != NULL ) {
    memset(af_msg, 0, sizeof(q3a_thread_af_msg_t));
    af_msg->type = MSG_AF_SET;
    af_msg->u.af_set_parm.type = AF_SET_PARAM_LENS_POS_FOCUS;
...
static void af_port_handle_awb_update(af_port_private_t * af_port,
  ...
  if (stats_update->awb_update.awb_custom_param_update.data &&
    ...
    if (af_msg != NULL) {
      ...
      af_msg->u.af_set_parm.type = AF_SET_PARAM_UPDATE_AWB_INFO;
      rc = q3a_af_thread_en_q_msg(af_port->thread_data, af_msg);
      if (!rc) {
...
static void af_port_handle_aec_update(af_port_private_t * af_port,
  ...
  if (af_msg != NULL) {
    ...
    af_port->fd_prio_data.preview_fps = (int) aec_info->preview_fps / 256;
  ...
  mct_event_module_t * mod_evt)
{
  ...
  if (af_msg != NULL) {
    ...
    af_port->actuator_info = sensor_info->actuator_info;
...
static boolean af_port_handle_send_fake_stats_on_ihist(af_port_private_t * af_port,
  ...
  do {
    ...
    if (af_port->af_initialized) {
      af_fdprio_process(&af_port->fd_prio_data,
  ...
  mct_event_module_t *mod_evt)
{
  ...
  q3a_thread_af_msg_t *af_msg = (q3a_thread_af_msg_t *) malloc(sizeof(q3a_thread_af_msg_t));
  ...
  af_port_private_t * af_port, mct_event_module_t * mod_evt)
{
  ...
  AF_LOW("send PDAF data to sensor, port =%p, event =%p",
    ...
    depth_map_conversion_status =
      depth_service_process(input, &(af_msg->u.af_set_parm.u.depth_service));
  ...
  mct_event_module_t * mod_evt)
{
  ...
  if (af_msg != NULL) {
    ...
    af_msg->type = MSG_AF_SET;
    /* Update message contents - first set param type*/
    af_msg->u.af_set_parm.type = AF_SET_PARAM_UPDATE_STATS_CAPS;
 ...
 * Return void
 **/
static void af_port_handle_intramod_event(mct_port_t *port,
  mct_event_module_t *mod_evt)
{
  af_port_private_t *af_port =
    (af_port_private_t *)(port->port_private);
  switch (mod_evt->type) {
    case MCT_EVENT_MODULE_STATS_PEER_AF_FOCUS_UPDATE:
        af_port_handle_peer_focus_info(port,
          (af_core_focus_info*)mod_evt->module_event_data);
      break;
    case MCT_EVENT_MODULE_STATS_PEER_CALI_DATA_UPDATE:
        AF_LOW("Received MCT_EVENT_MODULE_STATS_PEER_CALI_DATA_UPDATE");
        Q3A_MEMCPY((af_core_calibration_data*)mod_evt->module_event_data,
          &af_port->syscalib_data,
          sizeof(af_core_calibration_data));
    default:
        AF_ERR("Unknown Handle AF module event of type: %d", mod_evt->type);
      break;
  }
}
  ...
  mct_event_module_t *mod_evt)
{
  ...
  case MCT_EVENT_MODULE_STATS_GET_THREAD_OBJECT: {
    ...
    data->af_stats_cb = af_port_stats_done_callback;
 ...
 case MCT_EVENT_MODULE_IMGLIB_DCRF_OUTPUT: {
  ...
  case MCT_EVENT_MODULE_SET_FAST_AEC_CONVERGE_MODE: {
    ...
    if (af_msg != NULL) {
      ...
      af_msg->type = MSG_AF_SET;
      /* Update message contents - first set param type*/
      af_msg->u.af_set_parm.type = AF_SET_PARAM_FAST_AEC_ENABLED;
  ...
  af_set_parameter_t * set_parm, int mode)
{
  ...
  if ((cam_focus_mode_type)mode != af_port->af_mode) {
    AF_LOW("CAM-TYPE=%d Set Focus mode: %d old_mode: %d", af_port->dual_cam_sensor_info,mode, af_port->af_mode);
    mode_change = TRUE;
  }
...
static float af_port_get_hyper_focal(af_port_private_t *af_port)
{
  ...
  af_actuator_info_t *actr_info = &(af_port->actuator_info);
...
static boolean af_port_handle_set_focus_distance(mct_port_t *port, float focus_distance)
{
  ...
  if (af_port->af_mode != CAM_FOCUS_MODE_OFF && af_port->af_mode
    ...
    AF_ERR("not in CAM_FOCUS_MODE_OFF(Manual) mode, ignore the settings");
    return FALSE;
  }

  if (af_port->fd_prio_data.tuning_info == NULL){
    AF_ERR("tuning info null");
    return FALSE;
  }
...
static boolean af_port_handle_set_focus_manual_pos_evt(
  ...
  if (af_port->af_mode != CAM_FOCUS_MODE_OFF &&
      af_port->af_mode != CAM_FOCUS_MODE_MANUAL) {
    AF_ERR("not in CAM_FOCUS_MODE_OFF(Manual) mode,\
           ignore the settings");
    return FALSE;
  }
  ...
  af_set_parameter_t * parm)
{
  ...
  case AF_SET_PARAM_START: { // HAL3
    ...
    AF_LOW("CAM-TYPE=%d TRIGGER_START ID: %d, focus mode: %d, af_led_assist: %d",
      af_port->dual_cam_sensor_info,
  ...
  case AF_SET_PARAM_CANCEL_FOCUS: {// HAL3
    ...
    af_port->af_trans.trigger.af_trigger_id = parm->u.trigger.af_trigger_id;
    af_port->af_trans.trigger.af_trigger = parm->u.trigger.af_trigger;
    AF_LOW("CAM-TYPE=%d TRIGGER_CANCEL: ID: %d",
      af_port->dual_cam_sensor_info, af_port->af_trans.trigger.af_trigger_id);
    af_port->af_trigger_called = FALSE;
...
/** af_port_get_and_set_DualCalibrationData:
 *    @port: MCT Port
 *
 * Gets Data from Peer and Sets
 *
 * Return boolean
 **/
static boolean af_port_get_and_set_DualCalibrationData(mct_port_t *port) {
  af_port_private_t *af_port = (af_port_private_t *)(port->port_private);
  boolean rc = FALSE;
  AF_LOW("Retrieve Dual Calibration Data from peer!");
  af_core_calibration_data *p_calib_data = &af_port->syscalib_data;

  if(stats_util_post_intramode_event(port,af_port->intra_peer_id,
    MCT_EVENT_MODULE_STATS_PEER_CALI_DATA_UPDATE, p_calib_data)) {
    af_port_set_stereo_calibration_data(af_port, p_calib_data);
    rc = TRUE;
  }
  return rc;
}
...
static boolean af_port_link_to_peer(mct_port_t *port,
  ...
  AF_LOW("AF got MCT_EVENT_CONTROL_LINK_INTRA_SESSION to session %x", peer_identity);
  ...
  if (rc == FALSE) {
    ...
    af_port->intra_peer_id = peer_identity;
    af_port->dual_cam_sensor_info = link_param->type;
    MCT_PORT_INTRALINKFUNC(this_port)(peer_identity, this_port, peer_port);
    if(af_port->dual_cam_sensor_info == CAM_TYPE_AUX) {
      af_port_set_follow_mode(port,TRUE);
      af_port_get_and_set_DualCalibrationData(port);
    } else {
      af_port_set_follow_mode(port,FALSE);
    }
  }
  return rc;
}
...
static void af_port_handle_control_event(mct_port_t * port,
  ...
  case MCT_EVENT_CONTROL_SET_PARM: {
    ...
    /* If it's common params shared by many modules */
      ...
      if (common_param->type == COMMON_SET_PARAM_BESTSHOT) {
        ...
        af_port->thread_data->no_stats_mode = !common_param->u.stream_on;
  ...
  case MCT_EVENT_CONTROL_UNLINK_INTRA_SESSION: {
    AF_HIGH("AF got intra unlink Command");
    af_port->intra_peer_id = 0;
    af_port->dual_cam_sensor_info = CAM_TYPE_MAIN;
    af_port_set_follow_mode(port,FALSE);
  }
    break;
  case MCT_EVENT_CONTROL_HW_WAKEUP:{
    AF_LOW("CAM-TYPE=%d Got HW_WAKEUP Command",af_port->dual_cam_sensor_info);
    }
    break;
  case MCT_EVENT_CONTROL_HW_SLEEP: {
    AF_LOW("CAM-TYPE=%d AF got HW_SLEEP Command",af_port->dual_cam_sensor_info);
    }
    break;
  default: {
  }
...
static boolean af_port_intra_event(mct_port_t * port, mct_event_t * event)
{
  ...
  /* sanity check */
  if (!port || !event) {
    AF_ERR("NULL parameter");
    return FALSE;
  }

  AF_LOW("Received AF port event type: %d, dir: %d",
  ...
  if ((private->reserved_id & 0xFFFF0000) != (event->identity & 0xFFFF0000)) {
    AF_LOW("Identity does not match");
    return FALSE;
  }
  mct_event_module_t *mod_evt =
    (mct_event_module_t *)&(event->u.module_event);
  af_port_handle_intramod_event(port, mod_evt);
  return TRUE;
}
/** af_port_event:
