...
{
  asd_update_type->snow_scene_detection = ASD_DETECTION_ONLY_SCENE_INFO;
  asd_update_type->backlight_detection = ASD_DETECTION_ONLY_SCENE_INFO;
  asd_update_type->landscape_detection = ASD_DETECTION_ONLY_SCENE_INFO;
  asd_update_type->portrait_detection = ASD_DETECTION_ONLY_SCENE_INFO;
  asd_update_type->hazy_detection = ASD_DETECTION_ONLY_SCENE_INFO;
  asd_update_type->auto_hdr_detect = ASD_DETECTION_ONLY_SCENE_INFO;
  ...
  void *update)
{
  ...
  if (ASD_UPDATE == asd_out->type) {
    ...
    if (asd_out->asd_debug_data_size) {
      memcpy(private->asd_debug_data_array, asd_out->asd_debug_data_array,
        asd_out->asd_debug_data_size);
    }
  ...
  unsigned int session_id)
{
  ...
  asd_port_ext_set_update_type(&(ext_param->asd_update_type));
  return rc;
}
