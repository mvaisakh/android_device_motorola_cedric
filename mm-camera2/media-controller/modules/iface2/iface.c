    ...
    mct_event_t   *event)
{
  ...
  iface_session_t                  *prev_session         = NULL;
  iface_hw_stream_t                *prev_hw_stream       = NULL;
  ...
  /* dual camera support */
  if (iface->session_cnt > 1 && iface->total_rdi_cnt && iface->total_pix_cnt) {
    IFACE_HIGH("%s: session_cnt %d dual_rdi_count %d dual_pix_count %d\n",
      __func__, iface->session_cnt, iface->total_rdi_cnt,iface->total_pix_cnt);
    for (i = 0; i < IFACE_MAX_SESSIONS; i++) {
      prev_session = &iface->sessions[i];
      if (prev_session->session_id != session->session_id) {
        break;
      }
    }
    /* If the other active Session dont have any active streams,
    we don't need to do anything*/
    if (!prev_session->active_count) {
      goto resume_streamon;
    }
    /* We found a session with active streams, restart all streams*/
    for (i = 0; i < IFACE_MAX_STREAMS; i++) {
      prev_hw_stream = &prev_session->hw_streams[i];
      if (prev_hw_stream->hw_stream_id) {
        /* Found an active stream.. Lets stop it*/
        iface_util_axi_streamoff(iface,
                               prev_session,
                               1,
                               &prev_hw_stream->hw_stream_id);
      }
    }
    for (i = 0; i < IFACE_MAX_STREAMS; i++) {
      prev_hw_stream = &prev_session->hw_streams[i];
      if (prev_hw_stream->hw_stream_id) {
        /* Found an active stream.. Lets start it*/
       iface_util_axi_streamon(iface,
                               prev_session,
                               1,
                              &prev_hw_stream->hw_stream_id);
      }
  ...
  uint32_t  halt_frame_id)
{
  ...
  rc = iface_ispif_get_cfg_params_from_hw_streams(iface, iface_sink_port,
    session, num_hw_streams, hw_stream_ids, cfg_cmd, FALSE);
  ...
  rc = iface_ispif_get_cfg_params_from_hw_streams(iface, iface_sink_port,
    session, num_hw_streams, hw_stream_ids, cfg_cmd, TRUE);
  if (cfg_cmd->params.num == 0) {
    IFACE_ERR("ISPIF configure cmd is seriously wrong\n");
    rc = -1;
    goto error;
