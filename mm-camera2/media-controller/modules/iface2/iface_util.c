  ...
  iface_stream_t *stream)
{
  ...
  if ((stream->stream_info.fmt >= CAM_FORMAT_BAYER_MIPI_RAW_8BPP_GBRG &&
      stream->stream_info.fmt <= CAM_FORMAT_BAYER_MIPI_RAW_12BPP_BGGR) ||
      (stream->stream_info.fmt >= CAM_FORMAT_BAYER_MIPI_RAW_14BPP_GBRG &&
      stream->stream_info.fmt <= CAM_FORMAT_BAYER_MIPI_RAW_14BPP_BGGR) ||
      stream->stream_info.fmt == CAM_FORMAT_JPEG_RAW_8BIT ||
      stream->stream_info.fmt == CAM_FORMAT_META_RAW_8BIT ||
  ...
  */
  ...
  case CAM_CDS_MODE_ON:
    if (session->cds_curr_enb == 0) {
      CDBG_ERROR("%s:<cds_debug> HAL CDS mode on\n", __func__);
      *cds_enable = 1;
      cds_status = IFACE_CDS_STATUS_UPDATE;
    }
  ...
  case CAM_CDS_MODE_OFF:
    if (session->cds_curr_enb == 1) {
       CDBG_ERROR("%s:<cds_debug> HAL CDS mode off\n", __func__);
      *cds_enable = 0;
      cds_status = IFACE_CDS_STATUS_UPDATE;
    }
    break;
  ...
  if (cds_status == IFACE_CDS_STATUS_UPDATE) {
    CDBG_ERROR("%s:<cds_debug> take CDS! request enb = %d, curr enb = %d"
  ...
  iface_port_t *iface_sink_port, uint32_t session_id, uint32_t stream_id)
{
  ...
  for (i = 0; i < isp_resource_request.num_pix_stream
    ...
    rc = iface_util_fill_hw_stream_info_pix(reserving_hw_stream, iface_session,
      iface_sink_port, isp_pix_output, isp_resource_request.isp_id_mask, 0);
    if (rc < 0) {
  ...
  iface_port_t *iface_sink_port, uint32_t session_id, uint32_t stream_id)
{
  ...
  iface_util_dump_isp_resource_request(iface, & isp_resource_request);
  /*2016/10/20 Tianliang.Zhang get patch from QCT, optmz persist valuse should set as 0 {*/
  property_get("persist.camera.isp.clock.optmz", value, "0");
  /*2016/10/20 Tianliang.Zhang get patch from QCT, optmz persist valuse should set as 0 }*/
  ...
  if (clk_opt_enable) {
    ...
    for (i = 0; i < VFE_MAX; i++) {
      if (isp_resource_request.isp_id_mask & (1 << i)) {
        vfe_clk = iface_util_get_vfe_clk(split_info, sensor_out_info, i);
        if (vfe_clk > 0) {
          iface->isp_axi_data.axi_data[i].interface_clk_request[IFACE_INTF_PIX] =
            vfe_clk;
        } else {
  ...
  for (i = 0; i < isp_resource_request.num_pix_stream
    ...
    iface_session->num_hw_stream++;
    iface_session->cds_feature_enable &= isp_resource_request.cds_capable[i];
  }
  rc = iface_utill_fill_stats_stream_info(iface_session, &isp_resource_request,
  ...
  */
  ...
  if (split_info->is_split == 0) {
    ...
    */
    ...
    camif_hw_stream->left_pixel_ver_skip =
      iface_util_reverse_skip(skip_vert_pattern);
    camif_hw_stream->right_pixel_ver_skip =
      iface_util_reverse_skip(skip_vert_pattern);
  ...
  int                  axi_idx __unused)
{
  ...
  for (i = 0; i < axi_framedrop->framedrop.num_streams
    ...
    if (axi_framedrop->controllable_output) {
        iface_metadata.frame_drop.streamID[iface_metadata.
          frame_drop.num_streams++] =
            axi_framedrop->framedrop.streamID[i];
        CDBG_HIGH("%s: Send buf drop in meta for controllable stream (0x%x, fid: 0x%x)",
            __func__, axi_framedrop->framedrop.streamID[i],
            axi_framedrop->frame_id);
    } else {
      hw_stream =
        iface_util_find_hw_stream_in_session(
            session, axi_framedrop->framedrop.streamID[i]);
      if (hw_stream != NULL && !hw_stream->controllable_output) {
        for (j = 0; j < hw_stream->num_mct_stream_mapped &&
          j < IFACE_MAX_STREAMS; j++) {
          iface_metadata.frame_drop.streamID[iface_metadata.frame_drop.num_streams++] =
        ...
        CDBG_ERROR("%s: Error! Could not find hw_stream for session %d hw_stream_id 0x%x hw_stream %p\n",
          __func__, session_id, axi_framedrop->framedrop.streamID[i], hw_stream);
        continue;
      }
    }
  }
    ...
    for (j = 0; j < streams_desc->num_streams
      ...
      if (buf_alloc_info.stream_info[i].stream_type == streams_desc->type[j]) {
        ...
        buf_alloc_info.stream_info[i].fmt =
          ...
          CAM_FORMAT_YUV_420_NV12 : streams_desc->format[j];
        CDBG("%s:%d: stream type %d, stream fmt %d hw fmt %d\n", __func__, __LINE__,
...
static uint32_t iface_util_check_valid_frame(iface_hw_stream_t *hw_stream,
  cam_stream_ID_t *stream_ids, uint32_t *user_stream_id)
{
  ...
  *user_stream_id = 0;
  ...
  for (i = 0; i < stream_ids->num_streams && i < MAX_NUM_STREAMS; i++) {
    for (j = 0; j < IFACE_MAX_STREAMS; j++) {
      if (stream_ids->streamID[i] == mapped_mct_stream_id[j]) {
        (streams_num)++;
        stream_id = stream_ids->streamID[i];
      }
    }
  }
  ...
  iface_frame_divert_notify_t *divert_event)
{
  ...
  if (!hw_stream->divert_to_3a) {
    ...
    if (session->overflow_recovery_state !=
      ...
      is_buf_dirty = TRUE;
      memset(&error_bus_msg, 0, sizeof(error_bus_msg));
      error_bus_msg.sessionid =  user_stream->session_id;
      error_bus_msg.type = MCT_BUS_MSG_SEND_HW_ERROR;
      if (TRUE != mct_module_post_bus_msg(iface->module, &error_bus_msg)) {
        CDBG_ERROR("%s: HW error to bus failed\n", __func__);
      }
      goto ERROR;
    } else {
  ...
  uint32_t             session_id)
{
  ...
  for (i=0; i<axi_frame_drop->framedrop.num_streams
    ...
    hw_stream = iface_util_find_hw_stream_in_session(
        session, axi_frame_drop->framedrop.streamID[i]);
  ...
  int                  axi_idx)
{
  ...
  if (session->overflow_recovery_state ==
      IFACE_AXI_RECOVERY_STATE_OVERFLOW_DETECTED) {
    if (sof_event->frame_id <= session->sof_frame_id + 1) {
 ...
 uint32_t user_streamon_id, uint32_t is_streamon)
{
   ...
   if (is_streamon) {
     ...
     if (hw_stream->streamon_count == 0) {
       ...
       CDBG_ERROR("%s: left stream on count %d, skip hw streamoff\n",
...
void iface_util_setloglevel(void)
{
  ...
  CDBG_ERROR("%s:%d IFACE_Loglevel %d", __func__, __LINE__, g_ifaceloglevel);
  return;
}
