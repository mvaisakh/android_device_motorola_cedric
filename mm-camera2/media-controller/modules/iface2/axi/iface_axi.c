  ...
  triger_fetch_stream_t *action_data, uint32_t action_data_size)
{
  ...
  fe_cfg.output_stream_id = action_data->output_stream_id;
  ...
  iface_axi_stats_stream_t *axi_stats_stream)
{
  ...
  if (rc < 0) {
    ...
    iface_release_buf(axi_hw->buf_mgr, axi_stats_stream->bufq_handle);
  ...
  uint32_t                          in_params_size __unused)
{
  ...
  if (!skip_pattern->use_sw_skip) {
    ...
    rc = iface_axi_prepare_sw_frame_skip_info(axi_hw, skip_pattern);
    if (rc < 0) {
  ...
  uint32_t                          in_params_size __unused)
{
  ...
  if (!skip_pattern->use_sw_skip) {
    ...
    rc = iface_axi_prepare_sw_frame_skip_info(axi, skip_pattern);
    if (rc < 0) {
      CDBG("%s: no axi skip requested!\n",
                  __func__);
      return 0;
  ...
  uint32_t                     in_params_size __unused)
{
  ...
  update_cmd->update_info[0].user_stream_id =
...
static int iface_axi_reg_buf(iface_axi_hw_t *axi_hw, start_stop_stream_t *start_parm)
{
  ...
  for (i = 0; i < start_parm->num_streams; i++) {
    ...
    CDBG("%s: register user bufq 0x%x for user stream %d\n",
  ...
  boolean start)
{
  ...
  if (start == TRUE) {
    ...
    if (!(axi_hw->num_active_streams - start_stop_parm->num_streams)) {
    ...
    iface_axi_framedrop  *frame_drop)
{
  ...
  for (i = 0; i < VFE_AXI_SRC_MAX; i++) {
    ...
    if (rc < 0 || !hw_stream_info) {
      IFACE_ERR("Failed to report drop on axi_src %d frame_id %d", i,
        error_frame_id);
      continue;
    }

    frame_drop->framedrop.streamID[frame_drop->framedrop.num_streams++] =
    ...
    iface_axi_framedrop  *frame_drop)
{
  ...
  for (j = 0; j < IFACE_MAX_NUM_BUF_QUEUE; j++) {
    ...
    if (error_mask & (1 << bufq_idx)) {
      ...
      frame_drop->framedrop.streamID[
        frame_drop->framedrop.num_streams++] =
  ...
  if (rc >= 0) {
    ...
    case ISP_EVENT_STATS_NOTIFY:{
      ...
      if (rc < 0 || stats_notify_info.ack_flag == TRUE) {
        IFACE_ERR("enforce queue back stats buf!"
    ...
    case ISP_EVENT_FE_READ_DONE:{
      ...
      bus_msg.msg = (void *)&rd_done_event;
      bus_msg.size = sizeof(mct_bus_msg_isp_rd_done_t);
