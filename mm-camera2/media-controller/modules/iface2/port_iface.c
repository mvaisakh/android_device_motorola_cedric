...
static int port_iface_proc_mct_ctrl_cmd(mct_port_t *mct_iface_port, mct_event_t *event)
{
  ...
  case MCT_EVENT_CONTROL_STREAMON:
    ATRACE_BEGIN("iface:streamon fwd");
    ...
    if (ret < 0) {
      ...
      ATRACE_BEGIN("iface:streamon_to_thread");
  ...
  case MCT_EVENT_CONTROL_STREAMOFF:
    ATRACE_BEGIN("iface:streamoff fwd");
    ...
    if (rc == 0) {
      ...
      ATRACE_BEGIN("iface:streamoff_to_thread");
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_SENSOR_META_CONFIG: {
    ...
    CDBG("%s: identity %x num_meta %d is valid %d\n", __func__,
      ...
      sensor_meta_cfg->sensor_meta_info[0].is_valid);
    CDBG("%s: dim- %d x %d fmt %d", __func__,
      sensor_meta_cfg->sensor_meta_info[0].dim.width,
      sensor_meta_cfg->sensor_meta_info[0].dim.height,
      sensor_meta_cfg->sensor_meta_info[0].fmt);
