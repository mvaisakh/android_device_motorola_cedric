...
static int32_t flash_open(void **flash_ctrl, void *data)
{
  ...
  /* MODIFIED-BEGIN by yongzhi.li, 2016-09-21,BUG-2579753*/
  ctrl->test_dualled_setting= malloc(sizeof(test_dualled_setting_t));
  if (!ctrl->test_dualled_setting) {
    SERR("failed");
    rc = SENSOR_FAILURE;
    goto ERROR3;
  }
  memset(ctrl->test_dualled_setting, 0, sizeof(test_dualled_setting_t));
...
ERROR3:
  free(ctrl->rer->cfg);
  /* MODIFIED-END by yongzhi.li,BUG-2579753*/
ERROR2:
  free(ctrl->rer);
ERROR1:
  free(ctrl);
  ...
  sensor_submodule_event_type_t event, void *data)
{
  ...
  test_dualled_setting_t     *test_dualled_setting = NULL; // MODIFIED by yongzhi.li, 2016-09-21,BUG-2579753
  int32_t                    *mode = 0;
  ...
  test_dualled_setting = ((sensor_flash_data_t *)flash_ctrl)->test_dualled_setting; // MODIFIED by yongzhi.li, 2016-09-21,BUG-2579753
...
#ifdef __ANDROID__
    char value[200];
    property_get("persist.camera.flashderate", value, "1");
    if (atoi(value) > 0) {
  ...
  case LED_FLASH_SET_OFF:
    ...
    /* MODIFIED-BEGIN by yongzhi.li, 2016-09-21,BUG-2579753*/
    if(test_dualled_setting->enable){
      SERR("Test Dual Led set enable 0");
      test_dualled_setting->enable = 0;
    }
    break;
  case LED_FLASH_SET_TORCH:{

    /* MODIFIED-BEGIN by hongzhang, 2016-10-26,BUG-3232034*/
    //Same action with LED_FLASH_SET_PRE_FLASH
    translate_sensor_reg_setting_array(flash_settings,
    ...
    if (dual_led_setting->low_setting[0] == 0 &&
      ...
      /* MODIFIED-BEGIN by XuNan, 2016-10-31,BUG-3276581*/
      cfg.flash_current[0] = data ? *(int32_t *)data : dual_led_setting->low_setting[0];
      cfg.flash_current[1] = data ? *(int32_t *)data : dual_led_setting->low_setting[1];
      /* MODIFIED-END by XuNan,BUG-3276581*/
    } else {
      cfg.flash_current[0] = dual_led_setting->low_setting[0];
      cfg.flash_current[1] = dual_led_setting->low_setting[1];
    }
    /* MODIFIED-END by hongzhang,BUG-3232034*/

    if(test_dualled_setting->enable){
       cfg.cfg_type = CFG_FLASH_LOW;
       SERR("Test Dual Led is enable");
       cfg.flash_current[0] = test_dualled_setting->led1_setting;
       cfg.flash_current[1] = test_dualled_setting->led2_setting;
       test_dualled_setting->enable = 0;
    }
    break;
  }
  ...
  case LED_FLASH_SET_MAIN_FLASH: {
    ...
    if (dual_led_setting->high_setting[0] == 0 &&
      ...
      /* MODIFIED-BEGIN by XuNan, 2016-10-31,BUG-3276581*/
      cfg.flash_current[0] = data ? *(int32_t *)data : dual_led_setting->high_setting[0];
      cfg.flash_current[1] = data ? *(int32_t *)data : dual_led_setting->high_setting[1];
      /* MODIFIED-END by XuNan,BUG-3276581*/
    } else {
  ...
  /* MODIFIED-BEGIN by yongzhi.li, 2016-09-21,BUG-2579753*/
  case LED_FLASH_SET_TEST_DUAL_LED:{
    cam_test_dual_led_parm_t setting =  *(cam_test_dual_led_parm_t *)data;
    test_dualled_setting->enable = 1;
    test_dualled_setting->led1_setting = setting.led1;
    test_dualled_setting->led2_setting = setting.led2;
    SERR("Test Dual Led GOT TMP DATA IN FLASH:%d,%d,%d",
      test_dualled_setting->enable,
      test_dualled_setting->led1_setting,
      test_dualled_setting->led2_setting);
    }
    goto flash_process_Exit;
    /* MODIFIED-END by yongzhi.li,BUG-2579753*/
  default:
...
static int32_t flash_close(void *flash_ctrl)
{
  ...
  /* MODIFIED-BEGIN by yongzhi.li, 2016-09-21,BUG-2579753*/
  if (ctrl->test_dualled_setting != NULL) {
    /* Free test_dualled_setting */
    free(ctrl->test_dualled_setting);
    ctrl->test_dualled_setting = NULL;
  }
  /* MODIFIED-END by yongzhi.li,BUG-2579753*/
  ...
  dual_led_settings_t *dualled_setting)
{
    ...
    int32_t low1 = -1, low2 = -1, high1 = -1, high2 = -1;
  ...
  *dualled_setting = *flash_update;
  ...
  //pls set the value such as "11,20,100,200" for low1 2 high1 2
  property_get("persist.camera.dual_led_current", value, "");
  SLOW("DualLED %s: get the string is [%s]", __func__, value);
  ...
  if (strlen(value) > 0) {
	  ...
	  //get low1
	  current_str = strtok_r(value, ",", &saveptr);
	  if(NULL != current_str)
		  low1 = atoi(current_str);

	  //get low2
	  current_str = strtok_r(NULL, ",", &saveptr);
	  if(NULL != current_str)
		  low2 = atoi(current_str);

	  //get high1
	  current_str = strtok_r(NULL, ",", &saveptr);
	  if(NULL != current_str)
		  high1 = atoi(current_str);

	  //get high2
	  current_str = strtok_r(NULL, ",", &saveptr);
	  if(NULL != current_str)
		  high2 = atoi(current_str);

	  dualled_setting->low_setting[0] =COND_REPLACE(low1, dualled_setting->low_setting[0]);
	  dualled_setting->low_setting[1] =COND_REPLACE(low2, dualled_setting->low_setting[1]);
	  dualled_setting->high_setting[0] =COND_REPLACE(high1, dualled_setting->high_setting[0]);
	  dualled_setting->high_setting[1] = COND_REPLACE(high2, dualled_setting->high_setting[1]);
  }

      SLOW("DualLED2 %s: low12 high12 %d, %d, %d, %d\n", __func__,
