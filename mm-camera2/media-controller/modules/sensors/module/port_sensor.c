  ...
  void __attribute__((unused))*peer_caps, void *info)
{
  ...
  if((stream_info->dis_enable) && (stream_info->stream_type == CAM_STREAM_TYPE_VIDEO)) {
    if (bundle_info.s_bundle->max_width <
        (uint32_t)((stream_info->dim.width * 12) / 10))
      bundle_info.s_bundle->max_width =
        (uint32_t)((stream_info->dim.width * 12) / 10);
    if (bundle_info.s_bundle->max_height <
        (uint32_t)((stream_info->dim.height * 12) / 10))
  ...
  sensor_bundle_info_t *bundle_info, mct_event_module_t *event_module)
{
  ...
  SLOW("stats sensor_gain %f real_gain=%f lnct %d exp time %f", stats_update->aec_update.sensor_gain,
    stats_update->aec_update.real_gain, stats_update->aec_update.linecount, stats_update->aec_update.exp_time);
  ...
  if (stats_update->flag & STATS_UPDATE_AEC) {
    ...
    if (stats_update->aec_update.est_state == AEC_EST_NO_LED_DONE &&
       ...
       bundle_info->s_bundle->state = SENSOR_AEC_EST_NO_LED;
       SLOW("AEC EST DONE, no led needed");
      ...
      if (bundle_info->s_bundle->partial_flash_frame_skip
        ...
        if (bundle_info->s_bundle->regular_led_af == 0) {
          ...
          SLOW("Prepare for capture with LED AEC DONE");
     ...
     if(bundle_info->s_bundle->flash_rer_enable) {
      ...
      if(rc != TRUE)
        ...
        SLOW("AEC EST START");
        bundle_info->s_bundle->state = SENSOR_AEC_EST_START;
        if (bundle_info->s_bundle->led_calibration_mode !=
          CAM_LED_CALIBRATION_MODE_OFF) {
          SLOW("In led calibration mode: %d",
            bundle_info->s_bundle->led_calibration_mode);
          if (led_module_params->func_tbl.process != NULL) {
            rc = led_module_params->func_tbl.process(
              led_module_params->sub_module_private,
  ...
  mct_port_t *port, mct_event_t *event, sensor_bundle_info_t *bundle_info)
{
  ...
  case MCT_EVENT_MODULE_STATS_AEC_UPDATE: {
    ...
    /* normal case : call port_sensor_handle_aec_update()
       dual camera : call only when both sensors are streaming out */
    if (sensor_ctrl->is_dual_cam == FALSE ||
      (sensor_ctrl->is_dual_cam == TRUE &&
       sensor_ctrl->is_dual_streaming == TRUE)) {
  ...
  mct_port_t *port, mct_event_t *event, sensor_bundle_info_t *bundle_info)
{
  ...
  case MCT_EVENT_MODULE_STATS_AEC_UPDATE: {
    ...
    /* if the current session is YUV sensor,
       then apply AEC, otherwise just return */
    ...
    if (output_format == SENSOR_YCBCR) {
      ...
      sensor_util_post_intramode_event(module, event->identity,
        bundle_info->s_bundle->peer_identity,
        MCT_EVENT_MODULE_SENSOR_ADJUST_FPS, &dual_exp->adjusted_frame_duration);
    } else {
      SLOW("[dual] session %d : BAYER do nothing",
