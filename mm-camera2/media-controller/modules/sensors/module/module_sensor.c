  ...
  uint32_t identity)
{
  ...
  case CAM_INTF_PARM_TEST_DUALLED_VALUE: {
     cam_test_dual_led_parm_t tmp = *(cam_test_dual_led_parm_t *)event_control->parm_data;
     SERR("Test Dual Led GOT DATA TMP:%d,%d",tmp.led1,tmp.led2);
       module_sensor_params_t *led_module_params = NULL;
         led_module_params = s_bundle->module_sensor_params[SUB_MODULE_LED_FLASH];
         if (led_module_params->func_tbl.process != NULL) {
               rc = led_module_params->func_tbl.process(
                 led_module_params->sub_module_private,
                 LED_FLASH_SET_TEST_DUAL_LED, event_control->parm_data);
         }
     }
     break;
