...
#include "debug_lib.h"
#include "pdaf_lib.h"
#include "imx230_idol4pro_cn_lib.h"
  ...
  struct camera_i2c_reg_array reg_array[], int size, int res)
{
  ...
  p_regData->pd_offset_x_h = find_reg_data(reg_array, size, 0x3150);
  p_regData->pd_offset_x_l = find_reg_data(reg_array, size, 0x3151);
  p_regData->pd_offset_y_h = find_reg_data(reg_array, size, 0x3152);
  p_regData->pd_offset_y_l = find_reg_data(reg_array, size, 0x3153);
  p_regData->pd_area_w_h = find_reg_data(reg_array, size, 0x3154);
  p_regData->pd_area_w_l = find_reg_data(reg_array, size, 0x3155);
  p_regData->pd_area_h_h = find_reg_data(reg_array, size, 0x3156);
  p_regData->pd_area_h_l = find_reg_data(reg_array, size, 0x3157);

  for(i = 0; i < PD_FLEX_WIN_NUM*4*2; i++) {
    p_regData->reg_addr_flex_win[i] = find_reg_data(reg_array, size, 0x3158 + i);
  }
...
int32_t imx230_idol4pro_cn_pdaf_calculate_defocus
...
/**
 * FUNCTION: imx230_idol4pro_cn_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int imx230_idol4pro_cn_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < IMX230_IDOL4PRO_CN_MIN_AGAIN)
    {
        real_gain = IMX230_IDOL4PRO_CN_MIN_AGAIN;
        SLOW("IMX230_IDOL4PRO_CN Requested gain(%f) is lower than the sensor minimum(%d)",
            real_gain, IMX230_IDOL4PRO_CN_MIN_AGAIN);
    }
    else if (real_gain > IMX230_IDOL4PRO_CN_MAX_AGAIN)
    {
        real_gain = IMX230_IDOL4PRO_CN_MAX_AGAIN;
    }
...
/**
 * FUNCTION: imx230_idol4pro_cn_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float imx230_idol4pro_cn_register_to_real_gain(unsigned int reg_gain)
{
    float gain;

    if(reg_gain > IMX230_IDOL4PRO_CN_MAX_AGAIN_REG_VAL)
        reg_gain = IMX230_IDOL4PRO_CN_MAX_AGAIN_REG_VAL;
...
/**
 * FUNCTION: imx230_idol4pro_cn_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int imx230_idol4pro_cn_digital_gain_calc(
 float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = IMX230_IDOL4PRO_CN_MIN_DGAIN_REG_VAL;
    float real_dig_gain = IMX230_IDOL4PRO_CN_MIN_DGAIN;

    if(real_gain > IMX230_IDOL4PRO_CN_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = IMX230_IDOL4PRO_CN_MIN_DGAIN;
    }

    if(real_dig_gain > IMX230_IDOL4PRO_CN_MAX_DGAIN)
    {
        real_dig_gain = IMX230_IDOL4PRO_CN_MAX_DGAIN;
    }
  ...
  unsigned int line_count, sensor_exposure_info_t *exp_info, float s_real_gain)
{
    ...
    exp_info->reg_gain = imx230_idol4pro_cn_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain =
      imx230_idol4pro_cn_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      imx230_idol4pro_cn_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain =
      (float)exp_info->sensor_digital_gain / IMX230_IDOL4PRO_CN_MAX_DGAIN_DECIMATOR;
    exp_info->digital_gain =
      real_gain /(exp_info->sensor_real_gain * exp_info->sensor_real_dig_gain);
    exp_info->line_count = line_count;
    exp_info->s_reg_gain = imx230_idol4pro_cn_real_to_register_gain(s_real_gain);

    return 0;
}

/**
 * FUNCTION: imx230_idol4pro_cn_fill_exposure_array
 *
  ...
  __attribute__((unused)) int hdr_mode)
{
    ...
    if (hdr_mode == SENSOR_HDR_RAW) {
      ...
      reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_EXP_RATIO_ADDR;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_DIG_GAIN_GR_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_DIG_GAIN_GR_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_DIG_GAIN_R_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_DIG_GAIN_R_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_DIG_GAIN_B_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_DIG_GAIN_B_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_DIG_GAIN_GB_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_DIG_GAIN_GB_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;
  ...
  unsigned short awb_gain_b, struct camera_i2c_seq_reg_setting* reg_setting)
{
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_ABS_GAIN_R_WORD_ADDR;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX230_IDOL4PRO_CN_ABS_GAIN_B_WORD_ADDR;
