...
#include <stdio.h>
#include <string.h>
#include "s5k4h8_idol4pro_cn_lib.h"
#include "debug_lib.h"


/**
 * FUNCTION: s5k4h8_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int s5k4h8_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < s5k4h8_MIN_AGAIN)
    {
        real_gain = s5k4h8_MIN_AGAIN;
        SHIGH("s5k4h8 Requested gain(%f) is lower than the sensor minimum(%f)",
            real_gain, s5k4h8_MIN_AGAIN);
    }
    else if (real_gain > s5k4h8_MAX_AGAIN)
    {
        real_gain = s5k4h8_MAX_AGAIN;
    }
...
/**
 * FUNCTION: s5k4h8_register_to_real_gain
 *
 * DESCRIPTION: Calcuate the sensor exposure
 **/
static float s5k4h8_register_to_real_gain(unsigned int reg_gain)
{
    float real_gain;

    if (reg_gain <= s5k4h8_MIN_AGAIN_REG_VAL)
    {
        reg_gain = s5k4h8_MIN_AGAIN_REG_VAL;
        SHIGH("s5k4h8 Requested gain(%d) is lower than the sensor minimum(%f)",
            reg_gain, s5k4h8_MIN_GAIN);
    }
    else if (reg_gain > s5k4h8_MAX_AGAIN_REG_VAL)
    {
        reg_gain = s5k4h8_MAX_AGAIN_REG_VAL;
    }
...
/**
 * FUNCTION: s5k4h8_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int s5k4h8_digital_gain_calc(float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = s5k4h8_MIN_DGAIN_REG_VAL;
    float real_dig_gain = s5k4h8_MIN_DGAIN;

    if(real_gain > s5k4h8_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = s5k4h8_MIN_DGAIN;
    }

    if(real_dig_gain > s5k4h8_MAX_DGAIN)
    {
        real_dig_gain = s5k4h8_MAX_DGAIN;
    }
...
/**
 * FUNCTION: s5k4h8_calculate_exposure
 *
 * DESCRIPTION: Calcuate the sensor exposure
 **/
  ...
  __attribute__((unused)) float s_real_gain)
{
    ...
    exp_info->reg_gain = s5k4h8_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain = s5k4h8_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      s5k4h8_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
...
/**
 * FUNCTION: s5k4h8_fill_exposure_array
 *
