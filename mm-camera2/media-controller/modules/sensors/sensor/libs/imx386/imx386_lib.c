  ...
  struct camera_i2c_reg_array reg_array[], int size, int res)
{
  ...
  SERR(" Sandy saved res: %d, cur res: %d", saved_res, res);
  if(saved_res == res)
    return ret;
...
int32_t imx386_pdaf_calculate_defocus
  (void * arg1, signed long (*get_defocus)(void *input, void *output))
{
  ...
  SERR("Sandy PDAF calculate defocus, cur_res:%d",s_pdaf->cur_res);
  ...
  if(s_pdaf->eeprom_data != NULL) {
    ...
    for(i=0;i < dcc->XKnotNumSlopeOffset * dcc->YKnotNumSlopeOffset;i++) {
      /* restore to integer u7.9*/
      SlopeData[i] = (signed long)(dcc->SlopeData[i] * 512.0f);
  ...
  SLOW("PDAF AREA:x_offset:%d,y_offset:%d, w:%d, h:%d",
  ...
  for(j = 0;j < pd_win_y;j++)
    for(i = 0;i < pd_win_x;i++) {
      ...
      input.ConfidenceLevel
       ...
       SERR("Sandy input confidence leve=%d \n",s_pdaf->defocus[i + pd_win_x*j].df_conf_level);
     ...
     SERR("PDAF INPUT[%d][%d]: window: X_STA %d X_END %d, Y_STA %d Y_END %d",
       i, j,
      ...
      SERR("Sandy PDAF OUTPUT win:%d, defocus: %d, conf level: %d, "
        "phase diff: %f, DCC: %f",
...
/**
 * FUNCTION: imx386_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int imx386_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < IMX386_MIN_AGAIN)
    {
        real_gain = IMX386_MIN_AGAIN;
        SHIGH("IMX386 Requested gain(%f) is lower than the sensor minimum(%d)",
            real_gain, IMX386_MIN_AGAIN);
    }
    else if (real_gain > IMX386_MAX_AGAIN)
    {
        real_gain = IMX386_MAX_AGAIN;
    }
...
/**
 * FUNCTION: imx386_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float imx386_register_to_real_gain(unsigned int reg_gain)
{
    float gain;

    if(reg_gain > IMX386_MAX_AGAIN_REG_VAL)
        reg_gain = IMX386_MAX_AGAIN_REG_VAL;
...
/**
 * FUNCTION: imx386_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int imx386_digital_gain_calc(
 float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = IMX386_MIN_DGAIN_REG_VAL;
    float real_dig_gain = IMX386_MIN_DGAIN;

    if(real_gain > IMX386_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = IMX386_MIN_DGAIN;
    }

    if(real_dig_gain > IMX386_MAX_DGAIN)
    {
        real_dig_gain = IMX386_MAX_DGAIN;
    }
  ...
  unsigned int line_count, sensor_exposure_info_t *exp_info, float s_real_gain)
{
    ...
    exp_info->reg_gain = imx386_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain =
      imx386_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      imx386_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain =
      (float)exp_info->sensor_digital_gain / IMX386_MAX_DGAIN_DECIMATOR;
    exp_info->digital_gain =
      real_gain /(exp_info->sensor_real_gain * exp_info->sensor_real_dig_gain);
    exp_info->line_count = line_count;
    exp_info->s_reg_gain = imx386_real_to_register_gain(s_real_gain);

    return 0;
}

/**
 * FUNCTION: imx386_fill_exposure_array
 *
...
int sensor_fill_exposure_array(unsigned int gain,
  ...
  __attribute__((unused)) int luma_avg, __attribute__((unused)) unsigned int hdr_param,
  ...
  __attribute__((unused)) int is_hdr_enabled)
{
    ...
    reg_setting->reg_setting[reg_count].reg_data = (gain & 0x00000100) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX386_DIG_GAIN_GR_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX386_DIG_GAIN_GR_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX386_DIG_GAIN_R_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX386_DIG_GAIN_R_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX386_DIG_GAIN_B_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX386_DIG_GAIN_B_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX386_DIG_GAIN_GB_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX386_DIG_GAIN_GB_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;
