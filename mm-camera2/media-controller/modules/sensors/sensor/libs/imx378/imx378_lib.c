...
int32_t imx378_tron_pdaf_calculate_defocus // MODIFIED by yongzhi.li, 2016-09-02,BUG-2641941
  (void * arg1, signed long (*get_defocus)(void *input, void *output))
{
  ...
  SHIGH("PDAF calculate defocus, cur_res:%d",s_pdaf->cur_res);
  /* MODIFIED-END by yongzhi.li,BUG-2641941*/
  ...
  SHIGH("PDAF AREA:x_offset:%d,y_offset:%d, w:%d, h:%d", // MODIFIED by yongzhi.li, 2016-09-02,BUG-2641941
  ...
  for(j = 0;j < pd_win_y;j++)
    for(i = 0;i < pd_win_x;i++) {
     ...
     SHIGH("PDAF INPUT[%d][%d]: window: X_STA %d X_END %d, Y_STA %d Y_END %d", // MODIFIED by yongzhi.li, 2016-09-02,BUG-2641941
       i, j,
      ...
      SHIGH("PDAF OUTPUT win:%d, defocus: %d, conf level: %d, " // MODIFIED by yongzhi.li, 2016-09-02,BUG-2641941
        "phase diff: %f, DCC: %f",
...
/**
 // MODIFIED-BEGIN by yongzhi.li, 2016-09-02, BUG-2641941
 * FUNCTION: imx378_tron_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int imx378_tron_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < IMX378_TRON_MIN_AGAIN)
    {
        real_gain = IMX378_TRON_MIN_AGAIN;
        SHIGH("IMX378_TRON Requested gain(%f) is lower than the sensor minimum(%d)",
            real_gain, IMX378_TRON_MIN_AGAIN);
    }
    else if (real_gain > IMX378_TRON_MAX_AGAIN)
    {
        real_gain = IMX378_TRON_MAX_AGAIN;
        /* MODIFIED-END by yongzhi.li,BUG-2641941*/
    }
...
/**
 // MODIFIED-BEGIN by yongzhi.li, 2016-09-02, BUG-2641941
 * FUNCTION: imx378_tron_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float imx378_tron_register_to_real_gain(unsigned int reg_gain)
{
    float gain;

    if(reg_gain > IMX378_TRON_MAX_AGAIN_REG_VAL)
        reg_gain = IMX378_TRON_MAX_AGAIN_REG_VAL;
        /* MODIFIED-END by yongzhi.li,BUG-2641941*/
...
/**
 // MODIFIED-BEGIN by yongzhi.li, 2016-09-02, BUG-2641941
 * FUNCTION: imx378_tron_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int imx378_tron_digital_gain_calc(
 float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = IMX378_TRON_MIN_DGAIN_REG_VAL;
    float real_dig_gain = IMX378_TRON_MIN_DGAIN;

    if(real_gain > IMX378_TRON_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = IMX378_TRON_MIN_DGAIN;
    }

    if(real_dig_gain > IMX378_TRON_MAX_DGAIN)
    {
        real_dig_gain = IMX378_TRON_MAX_DGAIN;
        /* MODIFIED-END by yongzhi.li,BUG-2641941*/
    }
  ...
  unsigned int line_count, sensor_exposure_info_t *exp_info, float s_real_gain)
{
    ...
    /* MODIFIED-BEGIN by yongzhi.li, 2016-09-02,BUG-2641941*/
    exp_info->reg_gain = imx378_tron_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain =
      imx378_tron_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      imx378_tron_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain =
      (float)exp_info->sensor_digital_gain / IMX378_TRON_MAX_DGAIN_DECIMATOR;
    exp_info->digital_gain =
      real_gain /(exp_info->sensor_real_gain * exp_info->sensor_real_dig_gain);
    exp_info->line_count = line_count;
    exp_info->s_reg_gain = imx378_tron_real_to_register_gain(s_real_gain);

    return 0;
}

/**
 * FUNCTION: imx378_tron_fill_exposure_array
 // MODIFIED-END by yongzhi.li, BUG-2641941
 *
  ...
  __attribute__((unused)) int is_hdr_enabled)
{
    ...
    if(luma_delta != 0){
      ...
      reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_EXP_RATIO_ADDR; // MODIFIED by yongzhi.li, 2016-09-02,BUG-2641941
    ...
    /* MODIFIED-BEGIN by yongzhi.li, 2016-09-02,BUG-2641941*/
    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_DIG_GAIN_GR_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_DIG_GAIN_GR_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_DIG_GAIN_R_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_DIG_GAIN_R_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_DIG_GAIN_B_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_DIG_GAIN_B_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_DIG_GAIN_GB_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_DIG_GAIN_GB_ADDR + 1;
    /* MODIFIED-END by yongzhi.li,BUG-2641941*/
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;
  ...
  unsigned short awb_gain_b, struct camera_i2c_seq_reg_setting* reg_setting)
{
    ...
    /* MODIFIED-BEGIN by yongzhi.li, 2016-09-02,BUG-2641941*/
    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_ABS_GAIN_R_WORD_ADDR;
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX378_TRON_ABS_GAIN_B_WORD_ADDR;
    /* MODIFIED-END by yongzhi.li,BUG-2641941*/
