...
static int32_t sensor_write_init_settings(void *sctrl)
{
  ...
  for (index = 0; index < init_settings->size; index++) {
    ...
    if (ioctl(ctrl->s_data->fd, VIDIOC_MSM_SENSOR_GET_OTP_STATUS, &cfg) < 0) {
	SERR("LGLOTP jrdcam OTP ioctl failed\n");
    }else{
        SERR("LGLOTP jrdcam OTP done\n");
    }
  SLOW("exit");
  return SENSOR_SUCCESS;
}
...
CHROMATIX_NULL:
  SERR("chromatix for res_idx = %d special_mode_mask = %lld \
...
static int32_t sensor_get_frame_duration(void *sctrl, void *data)
{
  ...
  *frame_duration = (int64_t)NANO_SEC_PER_SEC *
    ((float)line_count * out_info_array->out_info[cur_res].line_length_pclk /
    vt_pix_clk);
...
static int32_t sensor_get_exposure_starttime(void *sctrl, void *data)
{
  ...
  line_readout = (int64_t)out_info_array->out_info[cur_res].x_output *
    NANO_SEC_PER_SEC / out_info_array->out_info[cur_res].op_pixel_clk;
  ...
  line_readout * (int64_t)out_info_array->out_info[cur_res].frame_length_lines;
