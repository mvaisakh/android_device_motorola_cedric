  ...
  case FIXED_GRID_WINDOW:
    ...
    if (params->type == 1) {
      if (win_num == 1) {
        ...
        ctrl->s_data->cur_pd_windows[0].x_sta = x_sta[i];
        ctrl->s_data->cur_pd_windows[0].y_sta = y_sta[i];
        ctrl->s_data->cur_pd_windows[0].x_end = x_end[i];
        ctrl->s_data->cur_pd_windows[0].y_end = y_end[i];
      } else {
...
#ifdef IMX378_FLIP_MIRROR
    s_pdaf->defocus[i].phase_diff = (float)phase_diff_raw / 16.0f*(-1);
#else
