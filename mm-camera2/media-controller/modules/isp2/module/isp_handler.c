  ...
  mct_event_t *event)
{
  ...
  ret = isp_util_dump_register(module, session_param, sof_event->frame_id,
    isp->isp_resource.isp_resource_info[session_param->hw_id[0]].
  ...
  mct_event_t *event)
{
  ...
  if (!bundle_params || session_param->fast_aec_mode == TRUE) {
    ...
    ISP_ERR("stream_off %d %d ide %x", event->u.ctrl_event.type,
 ...
 * Return: None
 **/
static void isp_handler_calculate_dis(
    isp_session_param_t *session_param,
    cam_dimension_t     *dim_update_info)
{
  int32_t camif_w =
    session_param->sensor_output_info.dim_output.width;
  int32_t camif_h =
    session_param->sensor_output_info.dim_output.height;

  dim_update_info->width =
      CEILING32((dim_update_info->width) +
      (uint16_t)(dim_update_info->width * DIS_MARGIN));
  dim_update_info->height =
      CEILING32((dim_update_info->height) +
      (uint16_t)(dim_update_info->height * DIS_MARGIN));

  if(dim_update_info->width > camif_w)
    dim_update_info->width = camif_w;
  if(dim_update_info->height > camif_h)
    dim_update_info->height = camif_h;
}
  ...
  for (i = 0; i < session_param->stream_port_map.num_streams; i++) {
    ...
    //Update the super dim info per axi stream
    {
      if (buf_alloc_info->stream_info[i].use_native_buffer) {
        ...
        if (super_dim_out->width  * stream_info_dim_in->height !=
            super_dim_out->height * stream_info_dim_in->width) {
          ISP_HIGH("%s: aspect ratio is not same\n",__func__);
          return FALSE;
        }
    ...
    mct_stream_info_t *stream_info)
{
  ...
  if (axi_src == CAMIF_RAW || axi_src > IDEAL_RAW) {
    ISP_ERR("axi src = %d (2 - CAMIF, 4/5/6 - RDI raw), "
