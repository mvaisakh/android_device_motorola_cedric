  ...
  unsigned int sessionid)
{
  ...
  if (isp->isp_resource.isp_resource_info
    [ISP_HW_0].isp_pipeline->is_camif_raw_op_fmt_supported) {
  ...
  isp_cap->max_frame_applying_delay = isp->isp_resource.isp_resource_info
  ...
  isp_cap->max_downscale_factor = isp->isp_resource.isp_resource_info
    [ISP_HW_0].isp_pipeline->max_scale_ratio;

  isp_cap->max_viewfinder_size.width = ISP44_VIEWFINDER_MAX_WIDTH;
  isp_cap->max_viewfinder_size.height = ISP44_VIEWFINDER_MAX_HEIGHT;
  ...
  unsigned int session_id)
{
  ...
  if (session_param->num_isp) {
    ISP_ERR("warning: ISP resource is not freed on time!");
