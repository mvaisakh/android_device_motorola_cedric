  ...
  isp_sub_module_t *isp_sub_module, void *data)
{
  ...
  ISP_ERR("right stripe offset %d overlap %d ",
  ...
  void *data)
{
  ...
  if (*frame_skip >= ltm_skip ) {
    ISP_ERR("Frame skip %x is covers min required by LTM", *frame_skip);
  } else {
    *frame_skip = ltm_skip;
  }
  ...
  void *data)
{
  ...
  mod->nLocalToneStrength = (int)Round((float)LINEAR_INTERPOLATION_LTM(
    ...
    mod->normal_idx.ratio));
  mod->fixed_content_low = (int)Round((float)LINEAR_INTERPOLATION_LTM(
    (float)LTM_CORE_TYPE(chromatix_ltm_normal,
    fixed_content_low, mod->normal_idx.tr_index),
    (float)LTM_CORE_TYPE(chromatix_ltm_normal,
    fixed_content_low, mod->normal_idx.tr_index + 1),
    mod->normal_idx.ratio));
  mod->fixed_content_high = (int)Round((float)LINEAR_INTERPOLATION_LTM(
    (float)LTM_CORE_TYPE(chromatix_ltm_normal,
  ...
  if (ASD_algo_data->backlit_scene_detect.backlit_LTM.enable) {
    ...
    nLocalToneStrength_backlit = (int)Round((float)LINEAR_INTERPOLATION_LTM(
      ...
      mod->backlit_idx.ratio));
   fixed_content_high_backlit = (int)Round((float)LINEAR_INTERPOLATION_LTM(
      (float)LTM_CORE_TYPE(chromatix_ltm_backlit,
      fixed_content_high, mod->backlit_idx.tr_index),
      (float)LTM_CORE_TYPE(chromatix_ltm_backlit,
      fixed_content_high, mod->backlit_idx.tr_index + 1),
      mod->backlit_idx.ratio));
   fixed_content_low_backlit = (int)Round((float)LINEAR_INTERPOLATION_LTM(
      (float)LTM_CORE_TYPE(chromatix_ltm_backlit,
 ...
 * Return TRUE on success and FALSE on failure
 **/
static boolean ltm44_attenuate_master_scale(int *ms_cur, ltm44_t* mod,
 ...
 float ratio)
{
  ...
  ltm44_attenuate_master_scale(master_scale, mod, ltm_strength);
  pack_tbl_to_lut_i32(master_scale, mod->hw_master_scale, 1.0f);

  if (mod->enable_adrc) {
    pack_tbl_to_lut_i32_shift(mod->ltm_adrc_algo.adrc_unpacked_shift_scale,
      mod->hw_shift_scale, scale_factor);
    pack_tbl_to_lut_i32(mod->ltm_adrc_algo.adrc_unpacked_master_curve,
      mod->hw_master_curve, 1.0f);
  } else {
    pack_tbl_to_lut_i32(mod->unpacked_shift_scale, mod->hw_shift_scale,
      scale_factor);
  }
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  for (i = 0, j = 0; j < LTM_HW_LUT_SIZE + 1; j++, i+=ADRC_LUT_SUB_SAMPLE) {
    mod->ltm_adrc_algo.adrc_unpacked_master_curve[j] =
      Round(LINEAR_INTERPOLATION_LTM((mod->ltm_adrc_algo.adrc_master_curve[i]/4.0f),
       (float)ltm->reservedData.master_curve[j],
       mod->ltm_adrc_algo.adrc_ltm_mix_rate));

    mod->ltm_adrc_algo.adrc_unpacked_master_scale[j] =
      MAX(mod->ltm_adrc_algo.adrc_master_scale[i]/2.0f,
       mod->unpacked_master_scale[j] * mod->ltm_adrc_algo.adrc_ltm_mix_rate);
  ...
  ltm44_setup_dark_bright_region_index(
    mod->ltm_adrc_algo.adrc_unpacked_master_scale, mod);
