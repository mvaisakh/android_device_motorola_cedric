  ...
  ISP_Y_ScaleCfgCmdType* y_cfg, int in_aspect_ratio)
{
  ...
  if (down_scaling_factor < SCALE_RATIO_LIMIT) {
    ...
    scaler->request_crop = 1;
    ISP_ERR("Entry %d y_cfg->hOut %d y_cfg->vOut %d",
...
void scaler44_calculate_dis(scaler44_t *scaler)
{
  ...
  if (scaler->dis_enable) {
    scaler->hw_stream_info.width =
      CEILING32((scaler->hw_stream_info.width_before_dis) +
      (uint16_t)(scaler->hw_stream_info.width_before_dis * DIS_MARGIN));
    scaler->hw_stream_info.height =
      CEILING32((scaler->hw_stream_info.height_before_dis) +
      (uint16_t)(scaler->hw_stream_info.height_before_dis * DIS_MARGIN));
 ...
 * Return: 0 always
 **/
boolean scaler44_init(void *mod_ctrl)
{
  scaler44_t *scaler = mod_ctrl;
  ...
  scaler44_t *scaler, void *data, uint32_t identity)
{
  ...
  case ISP_PRIVATE_HW_STREAM_LINKING: {
    boolean *hw_links = (boolean *)private_event->data;
    uint32_t i;
    for (i = 0; i < ISP_HW_STREAM_MAX; i++) {
      if (scaler->entry_idx == i) {
        scaler->is_linked = hw_links[i];
        break;
      }
    }
  }
