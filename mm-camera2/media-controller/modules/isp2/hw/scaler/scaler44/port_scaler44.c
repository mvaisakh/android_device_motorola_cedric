  ...
  mct_event_t *event)
{
  ...
  if (!scaler->dis_enable) {
    scaler->hw_stream_info.width = scaler->hw_stream_info.width_before_dis;
    scaler->hw_stream_info.height = scaler->hw_stream_info.height_before_dis;
  }
  ...
  mct_event_t *event)
{
  ...
  if (isp_cds_request->cds_request != TRUE) {
    ISP_ERR("isp cds request false, return true, skip cds");
    return TRUE;
  }
  ...
  scaler44_t *scaler, mct_event_control_parm_t *hal_param, uint32_t identity)
{
  ...
  case CAM_INTF_PARM_DIS_ENABLE: {
    ...
    ISP_DBG("hw_stream %d, is_linked %d, dis_enable %d",
      scaler->entry_idx, scaler->is_linked, scaler->dis_enable);
  }
    break;
  ...
  mct_port_direction_t direction, mct_port_caps_type_t caps_type)
{
  ...
  port_scaler44_data =
    (port_scaler44_data_t *)malloc(sizeof(port_scaler44_data_t));
  if (!port_scaler44_data) {
