  ...
  void *data)
{
  ...
  ISP_ERR("bg_rgn_width %d bg_rgn_height %d", bg_rgn_width, bg_rgn_height);
  ...
  void *data)
{
  ...
  if (!awb_config->bg_config.is_valid) {
    ISP_ERR("BG stats is not valid, dont update");
    return TRUE;
  }
...
boolean bg_stats44_init(isp_sub_module_t *isp_sub_module)
{
  ...
  bg_stats = (bg_stats44_t *)malloc(sizeof(bg_stats44_t));
  if (!bg_stats) {
    ISP_ERR("failed: bg_stats %p", bg_stats);
    PTHREAD_MUTEX_UNLOCK(&isp_sub_module->mutex);
    return FALSE;
